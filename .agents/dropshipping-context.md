...
| 2026-08-16 | Skill coverage audited against the plan's six phases. Two gaps filled: `ebay-listing-craft` (Cassini placement — item specifics are the dominant lever and were entirely uncovered) and `ebay-post-sale-ops` (seller standards, returns, cases — where accounts actually die). **Key threshold: under 400 transactions eBay evaluates over 12 months, so at 10 sales one defect = 10% defect rate, 5x the Below Standard limit, persisting a full year. Operating rule: resolve every dispute in the buyer's favour for the first ~50 sales.** Deliberately NOT built: account setup (one-time), ads (no budget), scaling (premature). |
